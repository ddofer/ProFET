...
MIN_PROTEIN_LENGTH_TAILED = 46 #If Terminal Tail split (27) Minimum length for protein sequences to be processed by FeatureGen().
...
def Get_Protein_Feat(seq,body_reduced_alph='ofer14',split_N=False,split_C=False,N_TAIL_REGION=27,C_TAIL_REGION=27):
    ...
    t0 = time.clock()
    features_dict.update (main_protein.GetSimpleFeatures(ParamScaleWindow=6)) #call on class method
    features_dict.update (main_protein.Get_ParamScales_Features(window=17))
    t1 = time.clock()
    time3 = t1-t0
    'N-Tail properties:'
    t0 = time.clock()
    ...
    t0 = time.clock()
    ofer14_seq = get_reduced_prot (seq=main_seq,alph_name='ofer14')
    # features_dict.update(ofer14_seq.GetkgramFreq(k=3))
    features_dict.update(ofer14_seq.GetKMirrorsFreq(k=3))
    t1 = time.clock()
    time5 = t1-t0
...
#    features_dict.update (hp2_seq.GetkgramCounts(k=5,UseVariableCombinations=True)) # name buggy?
    ...
    t0 = time.clock()
    selected3alph_4 = ['Charge_3','SolventA_3','NormVDWV_3'] #],'Hydrophobicity_3' ]
...
#        features_dict.update (seq_3.GetKMirrorsFreq(k=5))
    ...
    return [time1, time2, time3, time4, time5, time6, time7, 1], features_dict
...
def GenFeaturesFromFasta(files=None, Dirr = '.', classType = 'dir'): #, Multiclass=True):
            ...
            for record in SimpleFastaParser(fasta): #SeqIO.SimpleFastaParser(fasta):
              ...
              #   sequence = str(record.seq)
                ...
                if (((contain_illegals(sequence,ILLEGALS))==False) & (len(sequence)>45) & (all_feature_dict.get(seq_id) is None)):
                    times, all_feature_dict[seq_id]=Get_Protein_Feat(sequence)
                    if (classType == 'id'):
                        (all_feature_dict[seq_id])['classname'] = title.split()[1]
                    for i in [0,1,2,3,4,5,6,7]:
                        totTimes[i] = totTimes[i] + times[i]
                t1 = time.clock()
                time0 = t1-t0
    ...
    for i in [0,1,2,3,4,5,6]:
        totTimes[i] = totTimes[i] / totTimes[7]

    # print('protein feature extraction took, on average, ', totTimes)
...
## def featExt(directory, trainingSetFlag, classType, normFlag, normParams): ## Orig -
def featExt(directory, trainingSetFlag, classType, normFlag=True, normParams="."):
    ...
    'Removal of all zero columns - should not be done this way; must be for TEST as well as Train; disabled. Dan'
...
#    cType = 'file'
  ...
  # dirr = r'D:\SkyDrive\Dropbox\BioInformatics Lab\AA_Information\CODE\Feature_Extract\test_seq\Bench\LocTree3\bact'
  # import sys
  # dirr = sys.argv[1]
  dirr = str(input('Enter target directory'))
  # featExt()
  featExt(directory=dirr, trainingSetFlag=True, classType='id', normFlag=True, normParams=".")
