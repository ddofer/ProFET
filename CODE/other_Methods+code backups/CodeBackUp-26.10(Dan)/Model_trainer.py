...
def load_data(dataFrame="Feat_normalized.csv", dfType='file') :
    ...
    #TODO: We need option to predict using a tuned (parameters, etc') model classifier #Dan!
def getClassifier(classifierType):
    ...
    if (classifierType == 'forest'):
        model = RandomForestClassifier(n_jobs=-1, bootstrap=True, n_estimators=980,
                                        min_samples_leaf=1, min_samples_split =2,
                                        oob_score=True,max_features=0.15,
...
if __name__ == '__main__' :
    fname= r'E:\Dropbox\Dropbox\BioInformatics Lab\AA_Information\CODE\Feature_Extract\test_seq\Chap\trainingSetFeatures.csv'
#
    trainClassifier(filename=fname, normFlag=True, classifierType='forest', kbest=400, alpha=True, optimalFlag=True)
#    trainClassifier()
