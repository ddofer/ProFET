...
N_TAIL_REGION = 26
C_TAIL_REGION = 23
MIN_PROTEIN_LENGTH = 25 #Minimum length for protein sequences to be processed by FeatureGen().
...
'TODO: Calling N/C tail Split from prior method. Adjust prot.length accordingly!'
def Get_Protein_Feat(seq,body_reduced_alph='ofer14',split_N=False,split_C=False,N_TAIL_REGION=27,C_TAIL_REGION=27):
    ...
    if split_N is True:
        ...
        N_protein = ProtFeat.ProtFeat(N_seq,HAS_N=True,HAS_C=False)
    ...
    main_protein = ProtFeat.ProtFeat(main_seq,HAS_N=(not(split_N)),HAS_C=(not(split_C))) #new object of the proteinFeat class

    'Get simple protein features'
    features_dict.update (main_protein.GetSimpleFeatures(ParamScaleWindow=10)) #call on class method
    'N-Tail properties:'
    if split_N is True:
...
#    features_dict.update(ofer_Tail8_seq.GetKMirrorsFreq(k=2,getFreq=True))
...
#    # # # features_dict.update (hp3_seq.GetAA_Freq())
#    features_dict.update (hp3_seq.GetEntropy())
#    features_dict.update (hp3_seq.GetkgramCounts(k=4,UseVariableCombinations=True)) # UseVariableCombinations not Tested if working yet!!
...
#    # features_dict.update (sdm12_seq.GetAA_Freq())
#    features_dict.update (sdm12_seq.GetEntropy())
#    features_dict.update(sdm12_seq.GetKMirrorsFreq(k=2))

    selected3alph_3 = ['Charge_3','SolventA_3'] #],'Hydrophobicity_3' ,'NormVDWV_3']
...
###    for alph_3 in THREE_LETTER_ALPH_NAMES: #THREE_LETTER_ALPH_NAMES - from Aalphabets, ProFEAT groups
    for alph_3 in selected3alph_3:
        ...
        features_dict.update (seq_3.GetKMirrorsFreq(k=4))
...
def GenFeaturesFromFasta(files=None, Dirr = '.'): #, Multiclass=True):
    ...
    '''
    if files is None:
        print('GenFeaturesFromFasta - files is none. Getting files from Dirr')
        files=Get_Dirr_All_Fasta(Dirr) #DEfault gets files in curr. working directory
    #   print('Single file object passed. %s' %(files))

    all_feature_dict = {}
    def GetFeatFasta(fasta_file):
    ...
    if isinstance(files,str): #Single file to open
        print('Getting features from a single fasta file- %s' %(files))
        GetFeatFasta(files)
    else: #List of files
...
def get_features(files=None,returndf = True,saveCSV = True,Dirr = '.',\
  saveName = "Features_seq", multiClass=True,  multiClassLabels = ["one","two","three","four"]):
    ...
    def get_MultiClass_features():
        fasta_files = Get_Dirr_All_Fasta (Dirr)
        print ('Multiclass fasta_files list found: %s' %(fasta_files) )
        # i=0
        df = pd.DataFrame()
        ...
        for f in fasta_files:
            ...
            features = GenFeaturesFromFasta(files=f) #,Dirr=Dirr)
            ...
            # print(file.split())
            filename = str(f).strip('.fasta')
            dataframe['labels'] = filename
        ...
        # print(df)
        df.set_index('labels', append=True, inplace=True,drop=True)
        return df

    if multiClass == True:
      print ("Getting multiclass dataframes")
      features = get_MultiClass_features()
      dataframe = features
    else:
...
def UFF_features(files=None):
  '''
  Like save_features, but pivoted and gets just feature names - for use in
  ...
  We could maybe also do:
  http://stackoverflow.com/questions/22269331/how-to-prevent-the-index-in-a-pandas-dataframe-from-showing-up-in-excel?rq=1
  df2.to_csv("filename", index=0)
...
def normalize_zscore(df,saveCSV = True,filename='Feat_normalized.csv'):
  '''
...
if __name__=="__main__":
    ...
    # file_location ="./test_seq/NP/NP-Hormones"
##    file_location = "./test_seq/NP/NP+SP+Negs_BigBalanced"
#    file_location = "./test_seq/NP/NP+SP+Negs_SmallBalanced"
    file_location = "./test_seq/Chap"


    Training_Df = get_features(returndf = True,
      saveCSV = False, saveName = "raw_Train_Feat",
     multiClass=True, Dirr = file_location)

    'A much more methodical way to remove irrelevant features would be to remove those with variance 0. (Includes all 0 cols)'
    print("Removing all zero features")
    df=Training_Df
    # Training_Df =filterDF (Training_Df,removeZeroes=True,RemoveDuplicateCol=False)
    # Training_Df = Training_Df.groupby(Training_Df).filter(lambda x: x != 0)

    df_cleaned = df[df.columns[(df != 0).any()]] #All zero columns removed
...
##    df_cleaned = normalize_median(df_cleaned,saveCSV = False)
    ...
    z_df = normalize_zscore(df_cleaned,saveCSV = False,filename='Z+MAD_Feat.csv')

    z_df_2 = z_df[z_df.columns[(z_df != 0).any()]] #All zero columns removed
